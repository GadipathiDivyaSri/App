class Habit {
  ...
  void recalculateStreaks({DateTime? referenceDate}) {
    final ref = referenceDate ?? DateTime.now();
    final todayStr = '${ref.year}-${ref.month.toString().padLeft(2, '0')}-${ref.day.toString().padLeft(2, '0')}';
    
    final sortedComps = List<String>.from(completionHistory)..sort();
    final compSet = Set<String>.from(sortedComps);
    ...
    String earliest = sortedComps.first;
    if (startDate.isNotEmpty && startDate.compareTo(earliest) < 0) {
      earliest = startDate;
    }

    DateTime curDate;
    try {
      curDate = DateTime.parse(earliest);
    } catch (_) {
      curDate = ref.subtract(const Duration(days: 90));
    }

    final scheduledDays = <DateTime>[];
    while (!curDate.isAfter(ref)) {
      if (isScheduledForDate(curDate)) {
        scheduledDays.add(curDate);
      }
      curDate = curDate.add(const Duration(days: 1));
    }

    if (scheduledDays.isEmpty) {
      streakDay = compSet.contains(todayStr) ? 1 : 0;
    ...
    for (final day in scheduledDays) {
      final dStr = '${day.year}-${day.month.toString().padLeft(2, '0')}-${day.day.toString().padLeft(2, '0')}';
    ...
    // 2. Current streak: backwards scan from most recent scheduled day
    int current = 0;
    int idx = scheduledDays.length - 1;
    final lastScheduled = scheduledDays[idx];
    final lastDStr = '${lastScheduled.year}-${lastScheduled.month.toString().padLeft(2, '0')}-${lastScheduled.day.toString().padLeft(2, '0')}';

    // If today is a scheduled day and not completed yet, start from previous scheduled day
    if (lastDStr == todayStr && !compSet.contains(todayStr)) {
      idx--;
    }

    while (idx >= 0) {
      final day = scheduledDays[idx];
      final dStr = '${day.year}-${day.month.toString().padLeft(2, '0')}-${day.day.toString().padLeft(2, '0')}';
      if (compSet.contains(dStr)) {
        current++;
        idx--;
